...
document.addEventListener('DOMContentLoaded', () => {
  ...
  number.forEach((num) => {
    num.addEventListener('click', (even) => {
      clickNumber(even);
    });
  })
  ...
  const clearbaby = document.querySelector('.all-clear');

  clearbaby.addEventListener('click', clear);
  ...
  const symbolsOfLove = document.querySelectorAll('.operator');

  symbolsOfLove.forEach((sym) => {
    sym.addEventListener('click', (even) => {
      clickOperator(even);
    });
  })
    ...
    const peace = document.querySelector('.equal-sign');
    peace.addEventListener('click',performOperation);
});
