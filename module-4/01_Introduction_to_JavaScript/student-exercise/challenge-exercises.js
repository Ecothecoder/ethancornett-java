...
function iqTest(x){
    ...
    for (let i = 0; i<xArray.length; i++){
        if(xArray[i]%2===0){
            result=xArray.indexOf(xArray[i])+1;
        }
    }
    else if(oddCount===1)
    for (let i = 0; i<xArray.length; i++){
    ...
    console.log(count + ' ' + oddCount + " " +result);
    return result;
...
function titleCase(story, bonus){
    let storyTime = story.split(" ");
    let bonusArray;
...
if (!bonus){
    ...
    bonusArray = bonus.toLowerCase().split(" ");
}
console.log(bonusArray);
for (let i = 0; i<storyTime.length; i++){
    let test = storyTime[i].toLowerCase();
    console.log(test);
let temp = storyTime[i].substring(0,1).toUpperCase();
let temp2 = storyTime[i].substring(1,storyTime.length+2).toLowerCase();
let tempWord = (temp+temp2);
if (i<storyTime.length-1){
    tempWord+=" ";
}
storyTime[i]=tempWord;
      for (let j = 0; j<bonusArray.length; j++){
          console.log(bonusArray[j]);
        if (bonusArray[j]==test &&i!=0){
            storyTime[i]=bonusArray[j] + ' ';
            console.log(storyTime[i]);
        }
    }
