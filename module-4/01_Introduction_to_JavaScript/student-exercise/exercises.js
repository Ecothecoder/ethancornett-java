...
function lastDigit(x,y){
	let result = false;
	let xLast = x.toString().slice(-1);
	let yLast = y.toString().slice(-1);
	if (xLast === yLast){
		result = true;
	}
	return result;
...
*/
function seeColor(x){
	let color = x.toString().substring(0,3);
	let colorB = x.toString().substring(0,4);
	if(color === 'red'){
		return 'red';
	}
	else if (colorB === 'blue'){
...
*/
function fizzBuzz(x){
	let fizzyBuzzPants = 0;
	if (x%3===0 && x%5===0){
		fizzyBuzzPants ="FizzBuzz";
	}
	else if (x%3===0){
		fizzyBuzzPants="Fizz";
	}
