 ...
 * Document this function
 isSummer tests to see where summer is in the world, it checks the months/days
 to see whether or not summer is in the northern or southern hemisphere, defaulting to the northern 
 */
 ...
 */
 ...
 function makeHappy(ar){
   let hap = 'Happy '
  return ar.map((x) => hap + x);
 ...
 */
function getFullAddressesOfProperties(ar){

  return ar.map(x=> x.streetNumber + ' ' +  x.streetName +' ' + x.streetType +' ' + x.city +' ' + x.state +' ' + x.zip);
}
 ...
 */
 ...
 function findLargest(ar){
  return ar.reduce((acc, x) => x > acc? x : acc, 0);
 ...
 */
 ...
 function getSumOfSubArrayValues(ar){
   if (ar ==undefined||ar.length==0){
     return 0;
   }

  return ar.reduce((acc, x) =>acc.concat(x)).reduce((acc,x) => acc+x,0);
 }
