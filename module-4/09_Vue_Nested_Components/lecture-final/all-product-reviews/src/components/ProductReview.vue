...
export default {
  ...
  computed: {
    ...
    filteredReviews() {
      const filteredByRating = this.reviews.filter(review => this.filter === 0 || this.filter === review.rating);
      return filteredByRating.filter(review => review.review.includes(this.filterText));
