...
export default {
  ...
  computed: {
    ...
    filteredReviews() {
      const filterByRating = this.reviews.filter(review => this.filter === 0 || this.filter === review.rating);
      return filterByRating.filter(review => review.review.includes(this.filterText));
    },
