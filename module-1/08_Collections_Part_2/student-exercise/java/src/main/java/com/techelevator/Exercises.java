...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		Map<String, String> animalsAreCute = new HashMap<String, String>();
		// add all the data
		// check to see if its a key, if not return unknown
		animalsAreCute.put("RHINO", "Crash");
		animalsAreCute.put("GIRAFFE", "Tower");
		animalsAreCute.put("ELEPHANT", "Herd");
		animalsAreCute.put("LION", "Pride");
		animalsAreCute.put("CROW", "Murder");
		animalsAreCute.put("PIGEON", "Kit");
		animalsAreCute.put("FLAMINGO", "Pat");
		animalsAreCute.put("DEER", "Herd");
		animalsAreCute.put("DOG", "Pack");
		animalsAreCute.put("CROCODILE", "Float");
		if (animalsAreCute.get(animalName.toUpperCase()) == null)
			return "unknown";

		return animalsAreCute.get(animalName.toUpperCase());
	}
	...
	public Double isItOnSale(String itemNumber) {
		...
		if (itemsOnSale.get(itemNumber.toUpperCase()) == null)
			return 0.00;
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		int spareChange = peterPaul.get("Peter") % 2;
		if (peterPaul.get("Paul") < 1000) {
			peterPaul.put("Paul", (peterPaul.get("Peter") / 2 + peterPaul.get("Paul")));
			peterPaul.put("Peter", (peterPaul.get("Peter") / 2) + spareChange);
		}
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if (peterPaul.get("Peter") >= 5000 && peterPaul.get("Paul") >= 10000) {
			peterPaul.put("PeterPaulPartnership",
					((int) (peterPaul.get("Paul") / 4) + (int) (peterPaul.get("Peter") / 4)));
			peterPaul.put("Peter", (int) (peterPaul.get("Peter") * .75));
			peterPaul.put("Paul", (int) (peterPaul.get("Paul") * .75));
	 ...
	 * "m": "t", "n": "t"}/string length
	 */
	public Map<String, String> beginningAndEnding(String[] words) {
		Map<String, String> wordJumbo = new HashMap<String, String>();
		for (int i = 0; i < words.length; i++) {
			String arrayWord = words[i];
			String firstLetter = arrayWord.substring(0, 1);
			String lastLetter = arrayWord.substring(arrayWord.length() - 1, arrayWord.length());
			wordJumbo.put(firstLetter, lastLetter);
		}
		return wordJumbo;
	}
	...
	public Map<String, Integer> wordCount(String[] words) {
		Map<String, Integer> numberLetters = new HashMap<String, Integer>();

		for (String word : words) {
			if (numberLetters.get(word) == null) {
				numberLetters.put(word, 1);
			} else {
				int count = numberLetters.get(word);
				numberLetters.put(word, count + 1);
			}
		}
	 ...
	 * 63: 3, 77: 1, 99:2} integerCount([107, 33, 107, 33, 33, 33, 106, 107]) →
	 * {33: 4, 106: 1, 107: 3} integerCount([]) → {}
	 *
	 */
	public Map<Integer, Integer> integerCount(int[] ints) {
		Map<Integer, Integer> numberLetters = new HashMap<Integer, Integer>();
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		for (String check : words) {
			if (answers.get(check) == null) {
				answers.put(check, false);
			} else {
				answers.put(check, true);
			}
		}
		return answers;
	}
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		Map<String, Integer> oneMapToRuleThemAll = new HashMap<String, Integer>();
		Set<String> oneMaptoHold = new HashSet<String>();
		oneMaptoHold.addAll(mainWarehouse.keySet());
		oneMaptoHold.addAll(remoteWarehouse.keySet());
		for (String key : oneMaptoHold)
			if (mainWarehouse.containsKey(key) && remoteWarehouse.containsKey(key))
				oneMapToRuleThemAll.put(key, (mainWarehouse.get(key) + remoteWarehouse.get(key)));
			else if (mainWarehouse.containsKey(key))
				oneMapToRuleThemAll.put(key, mainWarehouse.get(key));
			else
				oneMapToRuleThemAll.put(key, remoteWarehouse.get(key));
		return oneMapToRuleThemAll;
	}
	 ...
	 * key = word do math on key
	 */
	public Map<String, Integer> last2Revisited(String[] words) {
		Map<String, Integer> numbersBeingCounted = new HashMap<String, Integer>();
		
		
		for (String key : words) {
			int count =0;
			for(int i=0; i<key.length()-2;i++) {
			String whyMeAgain = (key.substring(key.length()-2,key.length()));
			String whyHimAgain = key.substring(i,i+2);
			if (whyMeAgain.equals(whyHimAgain))
			{count ++;
			}}
		numbersBeingCounted.put(key,count);
		}
		return numbersBeingCounted;
	}
	...
	public List<String> distinctValues(List<String> stringList) {
		Set<String> returningList=new LinkedHashSet<String>();
		returningList.addAll(stringList);
		List<String>returnList =new LinkedList<String>();
		returnList.addAll(returningList);
		return returnList;
	}
