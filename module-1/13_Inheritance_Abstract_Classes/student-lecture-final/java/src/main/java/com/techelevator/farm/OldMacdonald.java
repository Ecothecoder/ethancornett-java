...
public class OldMacdonald {
	public static void main(String[] args) {
		...
		farmThings.add(new Cow());
		farmThings.add(new Cat());
		//farmThings.add(new FarmAnimal("generic farm animal", "meh", 1000)); this makes no sense
		
		for(Singable singAbout : farmThings) {
			
		/*	if (singAbout instanceof FarmAnimal) {
				((FarmAnimal) singAbout).goToSleep();
			} */
		...
		//just the cows
		for(Singable animal : farmThings) {
			if (animal instanceof Sellable && animal instanceof Cow) {
