...
public class NotificationController {
	...
	public String processNotificationSignupSubmission(@RequestParam(name="firstName") String first,
													 @RequestParam String lastName,
													 @RequestParam String emailAddress,
													 @RequestParam(defaultValue = "0") int age) throws FileNotFoundException, IOException {
		...
		try(FileOutputStream out = new FileOutputStream(getSignupLogFile(), true)) {
			...
			log.saveNotificationSignup(first, lastName, age, emailAddress);
		}
